...
merged_df <- merge(dat_alan, dat_joe, by = c("yyyy", "decade", "Zone", "largest.dhufish.kg"), all = TRUE)
glimpse(merged_df)
...
model <- aov(largest.dhufish.kg ~ Zone*decade, data = merged_df)
model1 <- lm(largest.dhufish.kg ~ yyyy, data = merged_df)
summary(model1)
