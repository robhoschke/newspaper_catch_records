...
library(dplyr)
library(corrr)
select(dt, distance,yyyy,largest.dhufish.kg)
corr_data <- select(dt, distance,yyyy,largest.dhufish.kg)
correlate(corr_data)
centr
...
for (i in 1:n_repeats) {
  ...
  # Fit the GAM model
  gam_model <- gam(distance ~ s(yyyy, by = Zone, k=4, bs="cr") ,
